...
export const getEtudiant = (classe, asc, skip) => (dispatch) => {
  ...
  try {
    fetch(
      `/api/etudiant/classe/${classe}/${asc}/skip=${(Number(skip) - 1) * 10}`
    )
      .then((response) => {
        if (!response.ok) {
          dispatch({
...
export const searchEtudiant = (classe, asc, searchVal, skip) => (dispatch) => {
  ...
  fetch(
    `/api/etudiant/search/${asc}/${classe}/${searchVal}/skip=${
      Number(skip) - 1
    }`, {
      headers: {
    ...
    .then((res) => {
      if (!res.ok) {
        ...
        dispatch({
          type: "SET_ETUDIANT_ERROR_MESSAGE",
          payload: "Vous ne pouvez pas effectuer cette recherche",
        });
      }
      return res.json();
    })
...
export const getTotalEtudiant = (classe, asc) => (dispatch) => {
  ...
  fetch(`/api/etudiant/nombre/total/asc/${asc}/classe/${classe}`, {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem("TOKEN")}`,
    },
  })
...
export const totalSearchEtudiant = (classe, asc, value) => (dispatch) => {
  ...
  fetch(`/api/etudiant/total/search/${asc}/${classe}/${value}`, {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem("TOKEN")}`,
    },
  })
